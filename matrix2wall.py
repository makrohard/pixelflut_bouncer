...
def new_offset(offset, half_x, half_y, direction_x, direction_y, res_x, res_y, step):
        ...
        if (offset[1] >= res_y - half_y):
            direction_y = -1
        return (offset[0] + step * direction_x, offset[1] + step * direction_y), direction_x, direction_y
...
def create_commands(color):
    commands = []
    for x, y in matrix:
        commands.append(f'PX %d %d %s' % (x, y, color))
